...
from domainbed.lib.misc import random_pairs_of_minibatches, ParamDict, random_split_minibatches
...
class AbstractDANN(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        self.update_count += 1
        all_x = torch.cat([x for x, y in minibatches]) #(batch_size*num_tdomains, )
        all_y = torch.cat([y for x, y in minibatches])
...
class DOML(Algorithm):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DOML, self).__init__(input_shape, num_classes, num_domains, hparams)
